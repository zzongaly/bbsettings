...
int main(int argc, const char *argv[]){
	...
	unsigned int channel_id = 0; 	// We'll alternate the memory positions 
					...
					// fill the other one.
	unsigned int last_channel_id = 0;
	...
	*/
	...
	while(!finish){
		...
		if(1 || buffer_count >= block_size*channels){
...
void init_adc(){
	...
	HWREG(0x44e0d040) |= (0x1 << 2);

	// Enable Step_ID_tag register. ADC_CTRL register
	...
	// 		range_check 	| DIFF_enable	| FIFO_select 	| SEL_INP_SWC 	| SEL_INM_SWC 	| Averaging 	| Mode
	HWREG(0x44e0d074) = 0x0000	| (0x0<<25)	| (0x1<<26)	| (0x02<<19)	| (0x00<<15) 	| (average<<2) 	| (0x01);
