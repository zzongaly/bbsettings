...
int main(int argc, const char *argv[]){
	...
	while(!finish){
		...
		if(buffer_count >= block_size*channels){
			...
			shared_ram[0] = 100+(channel_id * block_size * channels);
			shared_ram[1] = buffer_count;
			shared_ram[2] = channel_id;
		...
		for(i=0; i<fifo1_count; i++){
			...
			if(channel_id == 1){
				sample = data & 0xfff;
				buffer[(channel_id*block_size*channels) + buffer_count] = sample;
				buffer_count ++;
			}
			sample = data & 0xfff;
			switch(channel_id){
		...
		*/
		...
		if (x >2100 && x < 2200  && y>2020 && y < 2070){
			prevX = 0;
			prevY = 0;
			flag = 0;
			distance = 0;
...
void init_adc(){
	...
	HWREG(0x44e0d064) = 	0x0000 		| (0x0<<25)	| (0x0<<26) 	| (0x02<<19) 	| (0x00<<15) 	| (average<<2) 	| (0x01);
	...
	// 			range_check 	| DIFF_enable	| FIFO_select 	| SEL_INP_SWC 	| SEL_INM_SWC 	| Averaging 	| Mode
	HWREG(0x44e0d06c) = 	0x0000 		| (0x0<<25)	| (0x1<<26)	| (0x03<<19)	| (0x00<<15) 	| (average<<2) 	| (0x01);
	...
	// use fifo1, use channel 2. STEPCONFIG3 register
	// 			range_check 	| DIFF_enable	|| FIFO_select 	| SEL_INP_SWC 	| SEL_INM_SWC 	| Averaging 	| Mode
	HWREG(0x44e0d074) = 	0x0000 		| (0x0<<25)	| (0x1<<26)	| (0x01<<19)	| (0x00<<15) 	| (average<<2) 	| (0x01);
	...
	// 			range_check 	| DIFF_enable	|| FIFO_select 	| SEL_INP_SWC 	| SEL_INM_SWC 	| Averaging 	| Mode
	HWREG(0x44e0d084) = 	0x0000 		| (0x0<<25)	| (0x1<<26)	| (0x04<<19)	| (0x00<<15) 	| (average<<2) 	| (0x01);
